...
const header_text_config: clay.TextElementConfig = .{ .font_id = 1, .font_size = 16, .text_color = clay.Color.black };
...
fn headerButtonStyle(hovered: bool) clay.ElementDeclaration {
    return .{
        .layout = .{ .padding = clay.Padding.xy(16, 8) },
...
const dropdown_text_item_layout: clay.LayoutConfig = .{ .padding = clay.Padding.xy(8, 4) };
const dropdown_text_element_config: clay.TextElementConfig = .{ .font_size = 24, .text_color = clay.Color.white };

fn renderDropdownTextItem(index: usize) void {
    _ = index;
    clay.ui(.{ .layout = dropdown_text_item_layout, .background_color = clay.Color.rgb(180, 180, 180) })({
...
fn createLayout() clay.RenderCommandArray {
    return clay.layout()({
        clay.ui(.{ .id = clay.id("OuterContainer"), .layout = .{ .sizing = clay.Sizing.grow(.{}), .padding = clay.Padding.all(16), .child_gap = 16 }, .background_color = clay.Color.rgb(200, 200, 200) })({
            clay.ui(.{ .id = clay.id("SideBar"), .layout = .{ .layout_direction = .top_to_bottom, .sizing = .{ .width = clay.Sizing.Axis.fixed(300), .height = clay.Sizing.Axis.grow(.{}) }, .padding = clay.Padding.all(16), .child_gap = 16 }, .background_color = clay.Color.rgb(150, 150, 255) })({
                clay.ui(.{ .id = clay.id("ProfilePictureOuter"), .layout = .{ .sizing = .{ .width = clay.Sizing.Axis.grow(.{}) }, .padding = clay.Padding.all(8), .child_gap = 8, .child_alignment = .{ .y = .center } }, .background_color = clay.Color.rgb(130, 130, 255) })({
                    clay.ui(.{ .id = clay.id("ProfilePicture"), .layout = .{ .sizing = clay.Sizing.fixed(60) }, .image = .{ .image_data = &profile_picture, .source_dimensions = .{ .width = 60, .height = 60 } } })({});
                    clay.text(profile_text, .{ .font_size = 24, .text_color = clay.Color.black, .text_alignment = .right });
                });
            });

            clay.ui(.{ .id = clay.id("RightPanel"), .layout = .{ .layout_direction = .top_to_bottom, .sizing = .{ .width = clay.Sizing.Axis.grow(.{}), .height = clay.Sizing.Axis.grow(.{}) }, .child_gap = 16 } })({
                clay.ui(.{ .layout = .{ .sizing = .{ .width = clay.Sizing.Axis.grow(.{}) }, .child_alignment = .{ .x = .right }, .padding = clay.Padding.all(8), .child_gap = 8 }, .background_color = clay.Color.rgb(180, 180, 180) })({
            ...
            if (clay.getScrollContainerData(clay.id("MainContent"))) |scroll_data| {
                clay.ui(.{ .id = clay.id("ScrollBar"), .floating = .{ .attach_to = .element_with_id, .offset = .{ .y = -(scroll_data.scroll_position.y / scroll_data.content_dimensions.height) * scroll_data.scroll_container_dimensions.height }, .z_index = 1, .parent_id = clay.id("MainContent").id, .attach_points = .{ .element = .right_top, .parent = .right_top } } })({
                    clay.ui(.{ .id = clay.id("ScrollBarButton"), .layout = .{ .sizing = .{ .width = clay.Sizing.Axis.fixed(12), .height = clay.Sizing.Axis.fixed((scroll_data.scroll_container_dimensions.height / scroll_data.content_dimensions.height) * scroll_data.scroll_container_dimensions.height) } }, .background_color = if (clay.isPointerOver(clay.id("ScrollBar"))) clay.Color.rgba(100, 100, 140, 150) else clay.Color.rgba(120, 120, 160, 150), .corner_radius = clay.CornerRadius.all(6) })({});
                });
            }
        });
    });
...
fn updateDrawFrame(fonts: [*]rl.Font) void {
    ...
    if (rl.isMouseButtonDown(.left) and !scrollbar_data.mouse_down and clay.isPointerOver(clay.id("ScrollBar"))) {
        if (clay.getScrollContainerData(clay.id("MainContent"))) |scroll_container_data| {
