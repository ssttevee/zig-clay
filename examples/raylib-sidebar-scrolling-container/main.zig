            ...
            clay.ui(.{ .id = .id("RightPanel"), .layout = .{ .layout_direction = .top_to_bottom, .sizing = .{ .width = .grow(.{}), .height = .grow(.{}) }, .child_gap = 16 } })({
                ...
                clay.ui(.{
                    ...
                    clay.ui(.{
                        ...
                        clay.text("I'm an inline floating container.", .{ .font_size = 24, .text_color = .rgb(255, 255, 255) });
                    });

                    clay.text("Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt.", .{ .font_id = font_id_body_24, .font_size = 24, .text_color = .black });
                    ...
                    clay.text("Faucibus purus in massa tempor nec. Nec ullamcorper sit amet risus nullam eget felis eget nunc. Diam vulputate ut pharetra sit amet aliquam id diam. Lacus suspendisse faucibus interdum posuere lorem. A diam sollicitudin tempor id. Amet massa vitae tortor condimentum lacinia. Aliquet nibh praesent tristique magna.", .{ .font_size = 24, .line_height = 60, .text_color = .black, .text_alignment = .center });
                ...
                clay.ui(.{ .id = .id("ScrollContainer"), .layout = .{ .sizing = .{ .height = .fixed(200) }, .child_gap = 2 }, .scroll = .{ .vertical = true } })({
                    clay.ui(.{ .id = .id("FloatingContainer2"), .floating = .{ .attach_to = .parent, .z_index = 1 } })({
                        clay.ui(.{ .id = .id("FloatingContainerInner"), .layout = .{ .sizing = .{ .width = .fixed(300), .height = .fixed(300) }, .padding = .all(16) }, .background_color = .rgba(140, 80, 200, 200) })({
                            clay.text("I'm an inline floating container.", .{ .font_size = 24, .text_color = .white });
                        });
                    });
